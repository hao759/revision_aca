...
@Component({
  ...
  templateUrl: './demo-mutil.component.html',
  styleUrl: './demo-mutil.component.scss'
})
export class DemoMutilComponent {
  audit_id: any = 9999
  audit_date: any = 20250624
  shop_code: any = 1031799
  emp_code: any = 83789
  project_id: any = 30
  project_name: any = 'unime-ps'
  kpi_name: any = 10799
...
let shop_code=this.randomNineDigitNumber()
let emp_code=this.randomNineDigitNumber()
      this.service.uploadImage(this.project_id, this.audit_id, this.audit_date, shop_code, emp_code, this.project_name, this.kpi_name, this.randomNineDigitNumber(), event.srcElement.files[i]).subscribe((data: any) => {
        setTimeout(() => { this.callImg(yyyymmdd,shop_code,emp_code) }, 3000)
      }, (err: any) => {
        console.log(err)
...
this.shop_code = this.randomNineDigitNumber()
  ...
  callImg(detect_date: any,shop_code:any,emp_code:any) {

    this.service.uploadImageToDetect(this.project_id, this.audit_date, shop_code, emp_code, detect_date).subscribe((data: any) => {
      if(data.result!='ERROR')
      {
this.count_total_img--
      console.log("count_total_img", this.count_total_img)
      if (this.count_total_img>-1) {
        this.detect_productsTotal = data.data.detect_products
       // this.detect_photosTotal = 
        data.data.detect_photos.map((item: any) => {
          let img = this.sanitizer_var.bypassSecurityTrustResourceUrl(item.url_detect);
          // return {
          //   ...item,
          //   url_detect: img
          // }
          console.log("===detect_photosTotal==")
          this.detect_photosTotal.push({
            ...item,
            url_detect: img
          })
        })
        console.log("detect_photosTotal",this.detect_photosTotal)
        this.isLoading = false
      }
      }
      else
