...
@Component({
  ...
  imports: [RowComponent,
    ...
    NgSelectModule,
    JsonPipe,],
  templateUrl: './detect-ai-total-by-shop-umer.component.html',
  styleUrl: './detect-ai-total-by-shop-umer.component.scss'
})
export class DetectAiTotalByShopUmerComponent {
  constructor(
  ...
  from_date: Date = new Date('2025-06-25');
  ...
  filter(currentPage1: any) {
    this.listShop=[]
    this.service.GetAll_TotalByShop(Pf.DateToInt(this.from_date!), Pf.DateToInt(this.to_date!), 30, currentPage1).subscribe((data: any) => {
  ...
  async toggleDetails(item: any) {
    ...
    if (this.details_visible[item.shop_code].visible) {
      await this.service.GetDetail_Sum_TotalByShop(Pf.DateToInt(this.from_date!), Pf.DateToInt(this.to_date!), 30, item.shop_code).subscribe((datafinal: any) => {
...
async AwaitSave_Total(listData: any[],shop_code:any,auditdate:any) {
    ...
    return new Promise<any>((resolve, reject) => {
      this.detectAIService
        .ewo_detectai_detail_save_TotalByshop(data,shop_code,auditdate,30)
        .subscribe((data: any) => {
          if (data.result == EnumStatus.ok) {
